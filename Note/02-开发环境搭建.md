...
将准备好的 Maven 工程，方到本地目录下，并使用 IDEA 打开。其中聚合了三个工程。
...
| **序号** | **名称**            | **说明**                                                     |
| -------- | ------------------- | ------------------------------------------------------------ |
| 1        | sky-takeout-backend | maven父工程，统一管理依赖版本，聚合其他子模块                |
...
sky-server 模块中存放的是配置文件、配置类、拦截器、controller、service、mapper、启动类等
...
Lombok 提供了 @Builder 注解，用于使用链式编程的方式，封装一个实例对象。
...
- 进行负载均衡：所谓负载均衡，就是把大量的请求，按照指定的方式，均衡的分配给集群中的每台服务器。
...
| url_hash   | 依据 url 分配方式，这样相同的url会被分配到同一个后端服务 |

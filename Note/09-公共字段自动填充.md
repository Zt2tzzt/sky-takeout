...
针对于这些字段，现有的赋值方式为，在业务层（Service）中做数据补充：
...
- create_time、create_user，在记录插入时
- create_time、create_user、 update_time、update_user，在记录插入或修改时。

在实现公共字段自动填充，也就是在插入或者更新的时候，为指定字段赋予指定的值；

它的好处就是，可以统一对这些字段进行处理，避免了重复代码。

在上述的问题分析中，提到有四个公共字段，需要在新增、更新中进行赋值操作, 具体情况如下:
...
使用面型切面编程（AOP）的思想，为持久层（Mapper），添加逻辑。实现思路：

1. 自定义注解 `AutoFill`，用于标识需要进行公共字段自动填充的方法。
2. 在 Mapper 的方法上加入 `AutoFill` 注解。
...
定义一个每句类型 `OperationType`，表示数据库的操作类型：
...
> Spring Boot 项目测试类要在与启动类同层级，同报名+Tests 的情况下执行；
>
> 否则在 @SpringBootTest 注解添加启动类，比如：`@SpringBootTest(classes = SkyApplication.class)`

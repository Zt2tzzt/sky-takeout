...
新增菜品，其实就是将新增页面录入的菜品信息，插入到 dish 表；
...
- Spring Boot 会将中划线 `-` 转为映射为驼峰命名。
- 引用 application.dev.yml 配置文件中的属性。
...
定义一个配置类，在其中配置工具类 `AliOssUtil` 的 Bean 对象，并为其中四个属性，`endpoint`、`accessKeyId`、`accessKeySecret`、`bucketName` 初始化。
...
- 接口中 `flavors` 字段，是一个数组，这里用 `List<DishFlavor>` 类型接收。
...
数据库中新增了 dish 表、dish_flavor 表中xin'z